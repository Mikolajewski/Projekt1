...
bool Komunikacja::wczytaj_klucze_i_dane(Tablica& dane,string nazwa){
	fstream plik;
	string napis;
	int i,j,rozmiar;
	double pomocnicza;
	...
	if( plik.good() == true ){
		...
		liczba >> dane.dlugosc_tablicy;
      	...
      	dane.klucze=new string[dane.dlugosc_tablicy];
      	dane.klucze2=new string[dane.dlugosc_tablicy];
...
int Komunikacja::pobierz_int(){
	...
	while(blad==true){
		...
		istringstream liczba(napis);
		liczba >> i;
		if((i>32768||i<-32768)&&blad==false){
			cerr<<"Wpisana liczba nie jest int (-32 768,32 768)."<<endl<<"Wpisz liczbe jeszcze raz."<<endl<<endl;
			blad=true;
		}
		cin.clear();
		if(ujemna==true){
