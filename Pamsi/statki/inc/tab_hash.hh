...
public:
	 ...
	 */
	Tablica_hash<typ, klucz, dana>() {
		ilosc_elementow = 0;
		tablica = new typ*[17576];
		for (int i = 0; i < 17576; tablica[i] = NULL, i++)
			;
	}
	;
...
int Tablica_hash<typ, klucz, dana>::funkcja_hash(klucz haslo) {
	...
	for (i = 0; i < haslo.size() && i < ILOSC_SPRAWDZANYCH_ZNAKOW; i++) {
		komorka += pow(ILOSC_MOZLIWYCH_ZNAKOW, i)
				* ((int) haslo[i] - (int) PIERWSZY_ZNAK);
	}
	return komorka;
}

template<class typ, class klucz, class dana>
bool Tablica_hash<typ, klucz, dana>::dodaj(klucz haslo, klucz haslo2,
		dana wartosc) {
	typ* pomocnicza;
	typ* wskaznik;
	...
	if (tablica[komorka] != NULL) {
		...
		numer_na_liscie++;
		while (pomocnicza->zwroc_klucz() < haslo
				&& pomocnicza->zwroc_wskaznik() != NULL) { //szukanie miejsca w liscie do ktorego wpisac element
			pomocnicza = pomocnicza->zwroc_wskaznik();
			numer_na_liscie++;
		}
#if TYP == GRAF
		while (pomocnicza->zwroc_klucz() == haslo
				&& pomocnicza->zwroc_wskaznik() != NULL
				&& pomocnicza->zwroc_klucz2() < haslo2) { //dodatkowe wyszukiwanie jesli sa dwa hasla
			pomocnicza = pomocnicza->zwroc_wskaznik();
			numer_na_liscie++;
		}
		if (pomocnicza != NULL && pomocnicza->zwroc_klucz() == haslo
				&& pomocnicza->zwroc_klucz2() == haslo2) {
		...
		if (pomocnicza->zwroc_klucz() != haslo) {
			...
			if (pomocnicza->zwroc_wskaznik() == NULL
					&& pomocnicza->zwroc_klucz() <= haslo) {//jesli jest to koniec listy
			...
			if (pomocnicza->zwroc_wskaznik() == NULL
					&& pomocnicza->zwroc_klucz2() <= haslo2) {//jesli jest to koniec listy
				pomocnicza->wpisz_wskaznik(element);
				element->wpisz_ojciec(pomocnicza);
				element->wpisz_wskaznik(NULL);
			} else {
...
template<class typ, class klucz, class dana>
bool Tablica_hash<typ, klucz, dana>::znajdz1(klucz haslo) {
	typ* pomocnicza;
	int komorka;
...
template<class typ, class klucz, class dana>
bool Tablica_hash<typ, klucz, dana>::znajdz2(klucz haslo, klucz haslo2) {
	typ* pomocnicza;
	int komorka;
	int numer_na_liscie = 0;
	komorka = funkcja_hash(haslo);
...
typ* Tablica_hash<typ, klucz, dana>::nastepny(typ* element) {
	if (element->zwroc_wskaznik() != NULL
			&& element->zwroc_klucz()
					== element->zwroc_wskaznik()->zwroc_klucz()) {
...
bool Tablica_hash<typ, klucz, dana>::usun_element(klucz haslo, klucz haslo2) {
	...
	if (pomocnicza == NULL) {
		cerr << "Nie ma elementu o hasle: " << haslo << " i drugim hasle: "
				<< haslo2 << endl;
		return false;
	} else {                               					//usuwanie elementu
		cout << "Probuje usunac element o hasle: " << pomocnicza->zwroc_klucz()
				<< " i wartosci: " << pomocnicza->zwroc_klucz2() << endl;
