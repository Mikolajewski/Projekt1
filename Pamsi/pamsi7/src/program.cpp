...
bool Program::zapisz_dane_posortowane(char *nazwa){
	double i;
	Komunikacja dialog;
//	cout<<"Czy zapisac wynik obliczen? (t/n)"<<endl;
	if(1){	
		...
		plik_wyjsciowy<<5000<<endl;
		for(i=5000;i>0;i--)
			plik_wyjsciowy<<i<<endl;
		plik_wyjsciowy.close();
		cout<<"Dane zostaly poprawnie zapisane do pliku"<<endl;
	}	
	return true;
...
bool Program::stworz_plik_testowy(int ilosc,char* nazwa){
	cerr<<"Nie nadpisano metody tworzacej plik testowy dla tego programu.";
	return false;
}
...
clock_t Program::wstrzymaj_pomiar_czasu(){
	czas_wstrzymania=clock();
	return czas_wstrzymania;
}

clock_t Program::wznow_pomiar_czasu(){
	clock_t czas;
	czas=clock();
	czas_rozpoczecia+=czas-czas_wstrzymania;
	return czas;
}
